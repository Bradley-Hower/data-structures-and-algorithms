...
function transformToLis(obj){
  let thelist = [];
  for(let prop in obj){
    thelist.push(`<li>${prop}: ${obj[prop]}</li>`);
  }
  return thelist;
}
  ...
  input.map(e => {
    ...
    e.map(f => {
      if(Number.isInteger(f)){
        if(f % 5 === 0){
          let powernum = Math.pow(2, f);
          somenum.push(powernum);
        }
      }
    });
...
let findMaleAndFemale = (data) => {
  ...
  for(let char in data){
    if(data[char].gender === 'male' || data[char].gender === 'female'){
      if(arr.length < 1){
        arr.push(data[char].name);
      } else {
        arr.push(` and ${data[char].name}`);
      }
    }
  }
  return arr.join('');
...
let findShortest = (data) => {
  ...
  let lowchar = ['nonone'];
  for(let char in data){
    if(parseInt(data[char].height) < parseInt(lowestnum[lowestnum.length-1])){
      lowestnum.push(data[char].height);
      lowchar.push(data[char].name);
    }
  }
  return lowchar[lowchar.length-1];
};
