...
const sortStarWarsCharacters = (starWarsArr) => {
  let select = (a, b) => b.height - a.height;
  let sortedout = starWarsArr.sort(select);
  return sortedout;
};
...
const howMuchPencil = (str) => {
  let result = [];
  let str_arrayed = str.split('');
  result.push(str);
  while(str_arrayed.length > 0 ){
    str_arrayed.splice(0,1);
    result.push(str_arrayed.join(''));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for(const ingredient in recipe.ingredients){
    let round1 = recipe.ingredients[ingredient].indexOf(' ');
    let arr1 = recipe.ingredients[ingredient].slice(round1+1);
    let round2 = arr1.indexOf(' ');
    let arr2 = arr1.slice(round2+1);
    result.push(arr2);
  }
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  for(const ingredient in recipe.ingredients){
    let parcedchar = recipe.ingredients[ingredient].split(' ');
    for(let i=0; i < 2; i++){
      parcedchar.shift();
    }
    let joinedup = parcedchar.join(' ');
    result.push(joinedup);
  }
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  for(const action in recipe.steps){
    let parcedchar = recipe.steps[action].indexOf(' ');
    let slicedaction = recipe.steps[action].slice(0, parcedchar);
    result.push(slicedaction);
  }
  return result;
};
...
const removeEvenValues = (arr) => {
  for(const num in arr){
    if (num % 2 === 0){
      let tosplice = arr.indexOf(num);
      arr.splice(tosplice, 1);
    }
  }
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  let finishchar = strlen - numberOfCharacters;
  let finalstr = str.substr(0, finishchar);
  return finalstr;
};
