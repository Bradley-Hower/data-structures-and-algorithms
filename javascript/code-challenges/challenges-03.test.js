...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let higherstat = arr.filter( stats => stats.baseStat > minBaseStat);
  return higherstat;
};
...
const getStatName = (arr, minBaseStat) => {
  let higherstat = arr.filter( stats => stats.baseStat > minBaseStat);
  let statname = higherstat.map(element => element['stat']['name']);
  return statname;
};
...
const getCharactersWithoutChildren = (arr) => {
  let dink = arr.filter( element => element.children === undefined);
  return dink;
};
...
const evenOddNumericValues = (arr) => {
  let oddsorevens = [];
  let thenons = arr.filter(element => !isNaN(element));
  let thenonsnums = thenons.filter(element => typeof element === 'number');
  thenonsnums.map( (element) => {
    if(element % 2){
      oddsorevens.push('odd');
    } else if (element === 1){
      oddsorevens.push('odd');
    } else if (!(element % 2)) {
      oddsorevens.push('even');
    }
  });
  return oddsorevens;
};
