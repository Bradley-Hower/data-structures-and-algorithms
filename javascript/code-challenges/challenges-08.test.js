...
const sortByChildren = (charArray) => {
  let childrensort = charArray.sort((a,b) => {
    if(a.children.length - b.children.length === 0 || a.children.length - b.children.length === null || a.children.length - b.children.length === undefined){
      if(a.house < b.house){
        return -1;
      }
    } else {
  ...
  // let count = e => e + 1;
  // let elementcount = sortedbyname.reduce(count, 0);
  // let finalchildrensort = sortedbyname.reduce(elementcount, []);
  return childrensort;
};
...
const isCapitalized = (str) => {
  let regex = /[A-Z]\D\w*/g;
  let thewords = str.match(regex);
  if(regex.test(str)){
    return thewords;
  } else {
    return [];
  }
  // return anarray.push(select);
};
...
const citiesAtoJ = (arr) => {
  ...
  let newarray = arr.filter((e) => regex.test(e));
  return newarray;
};
...
let hangman = (str) => {
  ...
  let replacer = str.replace(regex, '_');
  return replacer;
};
