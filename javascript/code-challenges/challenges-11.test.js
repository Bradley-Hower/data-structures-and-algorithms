...
const longestString = (arr) => {
  ...
  for(let i = 1; i <= arr.length-1; i++){
    if(arr[i].length > arr[theone].length){
      let adding = i - theone;
      theone += adding;
    }
  }
...
const standardizePhoneNumbers = (arr) => {
  ...
  for(let i = 0; i<=arr.length-1; i++){
    ...
    for(let j = 0; j<=arr[i].length-1; j++){
      if(isNaN(arr[i].charAt(j)) || arr[i].charAt(j) === undefined || arr[i].charAt(j) === ' '){
        continue;
      } else {
        aphone.push(arr[i].charAt(j));
      }
    }
    let joined = aphone.join('');
    thephones.push(joined);
  }
  return thephones;
};
...
const onlyOddChars = (str) => {
  ...
  for(let i=0; i<=str.length-1; i++){
    if(i % 2 !== 0){
      theoddies.push(str.charAt(i));
    }
  }
  return theoddies.join('');
};
