...
const toTitleCase = (arr) => {
  ...
  for(let word in arr){
    let newword = [];
    let wordbeg = arr[word].slice(0,1);
    newword.push(wordbeg.toUpperCase());
    newword.push(arr[word].slice(1));
    let joined = newword.join('');
    newarr.push(joined);
...
let biggerThanLuke = (arr) => {
  let luke = arr.filter( e => e.name === 'Luke Skywalker')[0];
  let bigboys = arr.filter( e => parseInt(e.mass) > parseInt(luke.mass));
  let bigboysnames = [];
  bigboys.forEach( e => {
    bigboysnames.push( e.name);
  });
  let newnameoutput = bigboysnames.join(' - ');
...
const detectTicTacToeWin = (board) => {
  let C1 = board[0][0] === board[1][0] && board[0][0] === board[2][0];
  let C2 = board[0][1] === board[1][1] && board[0][1] === board[2][1];
  let C3 = board[0][2] === board[1][2] && board[0][2] === board[2][2];
  let R1 = board[0][0] === board[0][1] && board[0][0] === board[0][2];
  let R2 = board[1][0] === board[1][1] && board[1][0] === board[1][2];
  let R3 = board[2][0] === board[2][1] && board[2][0] === board[2][2];
  let X1 = board[0][0] === board[1][1] && board[0][0] === board[2][2];
  let X2 = board[0][2] === board[1][1] && board[0][2] === board[2][0];
  if(board[0][0] === '' || board[1][1] === '' || board [2][2] === ''){
    return false;
  } else {
    return C1 || C2 || C3 || R1 || R2 || R3 || X1 || X2;
  }
};
