...
const toLastNames = people => {
  let fullname = people.map(e => e.firstName + ' ' + e.lastName);
  return fullname;
};
...
const addValues = (arr) => {
  let summing = (a, b) => a + b;
  let summedelements = arr.reduce(summing, 0);
...
const addPurchases = (arr) => {
  let summing = (a,b) => a + b.purchasePrice;
  let summedup = arr.reduce(summing, 0);
  return summedup;
};
...
const countNumberOfElements = (arr) => {
  let count = e => e + 1;
  let countedup = arr.reduce(count, 0);
  return countedup;
};
...
const returnNames = (arr) => {
  ...
  let returnednames = arr.reduce(namefunc, []);
...
const reversedString = (str) => {
  ...
  let letterswap = (a, b) => b + a;
  let reversie = anarray.reduce(letterswap, '');
  return reversie;
};
...
const countNumberOfChildren = (arr) => {
  ...
  let childrenadded = anarray.reduce(haschild, 0);
